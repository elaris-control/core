...
function initNavRoutes({db,requireLogin,access}){
  ...
  r.get('/pages',(req,res)=>{try{const c=L.all(uid(req)).map(p=>{const raw=JSON.parse(p.instances_json||'[]');const instances=sanitizeInstances(req, raw);if(JSON.stringify(instances)!==JSON.stringify(raw)) U.run(p.name,p.icon,p.sort_order,JSON.stringify(instances),p.id,uid(req));return {...p,instances_json:JSON.stringify(instances),system:false,instances};});res.json({ok:true,pages:[...SYS,...c].sort((a,b)=>a.sort_order-b.sort_order)});}catch(e){res.status(500).json({ok:false,error:e.message});}});
  r.post('/pages',(req,res)=>{try{const{name,icon,instances}=req.body;if(!name)return res.status(400).json({ok:false,error:'missing_name'});const order=(M.get(uid(req)).m||0)+10;const safeInstances=sanitizeInstances(req, instances);const x=I.run(uid(req),name,icon||'page',order,JSON.stringify(safeInstances),Date.now());res.json({ok:true,id:x.lastInsertRowid});}catch(e){res.status(400).json({ok:false,error:e.message});}});
  r.put('/pages/:id',(req,res)=>{try{const{name,icon,instances,sort_order}=req.body;const p=G.get(Number(req.params.id),uid(req));if(!p)return res.status(404).json({ok:false,error:'not_found'});const safeInstances=instances!=null?sanitizeInstances(req, instances):sanitizeInstances(req, JSON.parse(p.instances_json));U.run(name!=null?name:p.name,icon!=null?icon:p.icon,sort_order!=null?sort_order:p.sort_order,JSON.stringify(safeInstances),Number(req.params.id),uid(req));res.json({ok:true});}catch(e){res.status(400).json({ok:false,error:e.message});}});
