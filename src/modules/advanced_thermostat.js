...
const { thermostatHandler, THERMOSTAT_MODULE, setManual, clearManual, setZoneManual, clearZoneManual } = require('../automation/advanced_thermostat');
...
function routes(app, ctx) {
  ...
  // POST /api/automation/advanced_thermostat/:id/control
  app.post('/api/automation/advanced_thermostat/:id/control', requireLogin, (req, res) => {
    try {
      const id = Number(req.params.id);
      const access = ensureUserModuleAccess(req, res, id, ({ def, ui }) => def?.id === 'advanced_thermostat' && !!ui.user_control);
      if (!access) return;
      const body = req.body || {};
      const out = {};
