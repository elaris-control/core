...
const MODULE = {
  ...
  inputs: [
    ...
    { key: "temp_boiler",  label: "Boiler Temperature",                type: "sensor", unit: "°C", required: true,
      description: "Temperature sensor in the solar storage tank/boiler. Used to calculate DT (solar − boiler) and stop the pump when the tank is hot enough." },
    ...
    { key: "pump_speed",   label: "Pump Speed (0–100%)",               type: "analog", unit: "%",  required: false,
      description: "Analog output (0–100%) connected to an inverter-driven pump. When mapped, enables variable-speed control to maintain the target DT." },
  ...
  setpoints: [
    ...
    { group: "basic", key: "profile",          label: "Pump control profile",  type: "select",  default: "basic",
      ...
      help: "basic: simple ON/OFF pump control based on DT thresholds. inverter_dt: variable-speed control that adjusts pump speed to maintain the target DT (requires pump_speed output)." },
    { group: "basic", key: "min_solar_temp",   label: "Min solar temp",        type: "number",  unit: "°C", step: 1,    default: 40,
      help: "The pump will not start unless the solar temp is above this temperature, even if DT is met. Prevents cold water from the solar panel cooling the boiler on cloudy days." },
    { group: "basic", key: "dt_on",            label: "DT ON (pump start)",    type: "number",  unit: "°C", step: 0.5,  default: 8,
    ...
    { group: "basic", key: "dt_off",           label: "DT OFF (pump stop)",    type: "number",  unit: "°C", step: 0.5,  default: 3,
    ...
    // ── Inverter / Speed ──────────────────────────────────────────────
    { group: "inverter", key: "dt_target",       label: "DT target",             type: "number",  unit: "°C", step: 0.5,  default: 10,
      help: "The ideal temperature difference (solar − boiler) to maintain. The speed controller adjusts pump speed up or down to keep DT near this value." },
    ...
    { group: "inverter", key: "start_delay_s",    label: "Start delay",           type: "number",  unit: "s",  step: 1,    default: 3,
      help: "Seconds to wait after the DT condition is met before starting the pump. Prevents false starts due to sensor noise." },
    { group: "inverter", key: "stop_delay_s",     label: "Stop delay",            type: "number",  unit: "s",  step: 1,    default: 3,
      help: "Seconds to wait after the DT drops below dt_off before stopping the pump. Adds damping to the stop decision." },
    ...
    { group: "inverter", key: "ema_alpha",         label: "DT filter (EMA α)",    type: "number",  unit: "",   step: 0.05, default: 0.25,
      help: "Exponential moving average smoothing factor for the DT signal (0.0–1.0). Lower = smoother but slower response. Higher = faster but noisier. 0.25 is a good starting point." },
    { group: "inverter", key: "map_gain",          label: "Mapping gain (no-PI)", type: "number",  unit: "",   step: 0.5,  default: 6,
      help: "Proportional gain for simple DT-to-speed mapping when PI controller is disabled. Speed = (DT / dt_target) × gain × 50%." },
    { group: "inverter", key: "use_pi",            label: "Use PI controller",    type: "select",  default: "0", options: ["0","1"],
      help: "0 = simple proportional mapping (recommended for most systems). 1 = PI controller for tighter DT regulation (requires tuning Kp and Ki)." },
    ...
    { group: "inverter", key: "anti_cycle_s",      label: "Anti-cycle lockout",   type: "number",  unit: "s",  step: 5,    default: 60,
      help: "Minimum time the pump must stay OFF after stopping before it can start again. Prevents rapid on/off cycling when DT is near the threshold." },
    { group: "inverter", key: "manual_override",   label: "Manual override",      type: "select",  default: "0", options: ["0","1"],
      help: "1 = Force the pump to run at the manual speed, ignoring all DT logic. Useful for priming, testing, or forced circulation." },
