// public/js/modules/mod_scheduled_motion.js

function analyzeScheduledMotionMappings(currentMappings = {}) {
  ...
  if (!pir)   issues.push({ severity: 'bad', message: 'Map a PIR sensor — motion trigger within the schedule window.' });
  if (!switchDi) issues.push({ severity: 'info', message: 'No wall switch mapped. Control via PIR/schedule and dashboard only.' });
  ...
  return { relay, relay2, relay3, relay4, switchDi, pir, issues, readiness, relayCount: relays.length };
}

function renderScheduledMotionCommissioningSummary(currentMappings = {}) {
  const a = analyzeScheduledMotionMappings(currentMappings);
  ...
  ].filter(Boolean).join('') || `<span class="thermo-zone-pill"><strong>No inputs</strong><span class="mini">Map PIR sensor.</span></span>`;
  const issuesHtml = a.issues.length
    ...
    : `<div class="thermo-issues"><div class="thermo-issue sev-info"><strong>Ready:</strong> Scheduled + Motion mapping looks good. Set the schedule window in setpoints.</div></div>`;
  return `
    <div class="thermo-summary">
      <div class="thermo-summary-head">
        <div>
          <div class="thermo-summary-title">Commissioning summary</div>
          <div class="thermo-summary-sub">Scheduled + Motion — PIR triggers only during schedule window.</div>
        </div>
      ...
      <div class="thermo-sum-grid">
        ...
        <div class="thermo-stat"><div class="thermo-stat-k">PIR sensor</div><div class="thermo-stat-v">${a.pir ? 'Mapped' : '—'}</div></div>
...
function updateScheduledMotionCommissioningSummary(currentMappings = null) {
  const box = document.getElementById('scheduledMotionCommissioningSummary');
  if (!box) return;
  box.innerHTML = renderScheduledMotionCommissioningSummary(currentMappings || collectCurrentWizardMappings());
}

function renderScheduledMotionSummary(inst, settings, live) {
  ...
  parts.push(`<div style="display:flex;gap:6px;flex-wrap:wrap;margin:0 0 8px">
    ...
    ${state.motion_active ? renderBadge('MOTION', '#f5c842', 'rgba(245,200,66,.25)') : ''}
    ${state.schedule_active != null ? renderBadge(state.schedule_active ? 'IN SCHEDULE' : 'OFF SCHEDULE', state.schedule_active ? '#22d97a' : 'var(--muted)', state.schedule_active ? 'rgba(34,217,122,.25)' : 'var(--line)') : ''}
...
registerModule('scheduled_motion', {
  hasAuto: true,
  summaryBoxId: 'scheduledMotionCommissioningSummary',
  updateCommissioningSummary(m) { updateScheduledMotionCommissioningSummary(m); },
  renderSummary(inst, s, live) { return renderScheduledMotionSummary(inst, s, live); },
});
