// public/js/modules/mod_motion_light.js

function analyzeMotionLightMappings(currentMappings = {}) {
  ...
  if (!pir)   issues.push({ severity: 'bad', message: 'Map a PIR motion sensor — it is the main trigger for this module.' });
  if (!switchDi) issues.push({ severity: 'info', message: 'No wall switch mapped. Control via PIR and dashboard only.' });
...
function renderMotionLightCommissioningSummary(currentMappings = {}) {
  const a = analyzeMotionLightMappings(currentMappings);
  ...
  const issuesHtml = a.issues.length
    ...
    : `<div class="thermo-issues"><div class="thermo-issue sev-info"><strong>Ready:</strong> Motion Light mapping looks good.</div></div>`;
  return `
    <div class="thermo-summary">
      <div class="thermo-summary-head">
        <div>
          <div class="thermo-summary-title">Commissioning summary</div>
          <div class="thermo-summary-sub">Motion Light — PIR triggers light with auto-OFF timer.</div>
        </div>
...
function updateMotionLightCommissioningSummary(currentMappings = null) {
  const box = document.getElementById('motionLightCommissioningSummary');
  if (!box) return;
  box.innerHTML = renderMotionLightCommissioningSummary(currentMappings || collectCurrentWizardMappings());
}

function renderMotionLightSummary(inst, settings, live) {
  ...
  parts.push(`<div style="display:flex;gap:6px;flex-wrap:wrap;margin:0 0 8px">
    ...
    ${state.source && state.source !== 'idle' ? renderBadge(String(state.source).replace(/_/g,' ').toUpperCase(), '#8b9cf4', 'rgba(139,156,244,.25)') : ''}
  </div>`);
...
registerModule('motion_light', {
  hasAuto: true,
  summaryBoxId: 'motionLightCommissioningSummary',
  updateCommissioningSummary(m) { updateMotionLightCommissioningSummary(m); },
  renderSummary(inst, s, live) { return renderMotionLightSummary(inst, s, live); },
});
