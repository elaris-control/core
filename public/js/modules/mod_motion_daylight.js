// public/js/modules/mod_motion_daylight.js

function analyzeMotionDaylightMappings(currentMappings = {}) {
  ...
  if (!pir)   issues.push({ severity: 'bad', message: 'Map a PIR sensor — motion trigger.' });
  if (!lux)   issues.push({ severity: 'bad', message: 'Map a lux sensor — darkness check before triggering.' });
  if (!switchDi) issues.push({ severity: 'info', message: 'No wall switch mapped. Control via PIR/lux and dashboard only.' });
  ...
  return { relay, relay2, relay3, relay4, switchDi, pir, lux, issues, readiness, relayCount: relays.length };
}

function renderMotionDaylightCommissioningSummary(currentMappings = {}) {
  const a = analyzeMotionDaylightMappings(currentMappings);
  ...
  ].filter(Boolean).join('') || `<span class="thermo-zone-pill"><strong>No inputs</strong><span class="mini">Map PIR and lux sensor.</span></span>`;
  const issuesHtml = a.issues.length
    ...
    : `<div class="thermo-issues"><div class="thermo-issue sev-info"><strong>Ready:</strong> Motion + Daylight mapping looks good.</div></div>`;
  return `
    <div class="thermo-summary">
      <div class="thermo-summary-head">
        <div>
          <div class="thermo-summary-title">Commissioning summary</div>
          <div class="thermo-summary-sub">Motion + Daylight — PIR triggers only when dark.</div>
        </div>
...
function updateMotionDaylightCommissioningSummary(currentMappings = null) {
  const box = document.getElementById('motionDaylightCommissioningSummary');
  if (!box) return;
  box.innerHTML = renderMotionDaylightCommissioningSummary(currentMappings || collectCurrentWizardMappings());
}

function renderMotionDaylightSummary(inst, settings, live) {
...
registerModule('motion_daylight', {
  hasAuto: true,
  summaryBoxId: 'motionDaylightCommissioningSummary',
  updateCommissioningSummary(m) { updateMotionDaylightCommissioningSummary(m); },
  renderSummary(inst, s, live) { return renderMotionDaylightSummary(inst, s, live); },
});
