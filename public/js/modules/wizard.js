...
async function saveModule(existingId=null) {
  ...
  (selectedDef?.inputs||[]).forEach(input=>{
    const sel = document.getElementById("map_"+input.key);
    if (sel && !sel.disabled && sel.value) mappings[input.key] = Number(sel.value);
    else if (id && sel) mappings[input.key] = 0; // explicitly remove when editing, including disabled/unticked rows
  });
...
Save anyway?`)) return;
  ...
  try {
    if (id) {
      await api(`/api/modules/instances/${id}/mappings`,{method:"PATCH",body:JSON.stringify({ name, mappings })});
