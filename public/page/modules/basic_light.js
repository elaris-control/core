// ── public/page/modules/basic_light.js ────────────────────────────────
// Basic Light module renderer — uses shared W.* widgets.
// ───────────────────────────────────────────────────────────────────────

(function(){
  'use strict';

  var MODULE_ID = 'basic_light';

  function esc(s){ return String(s==null?'':s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;'); }

  function renderBasicLight(inst){
    ...
    return Promise.all([statusData, liveData]).then(function(results){
      ...
      var h = '';
      h += W.cardHeader('Basic Light', state.status || 'Idle',
  ...
  window.renderBasicLightModule = renderBasicLight;
