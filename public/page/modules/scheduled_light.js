// ── public/page/modules/scheduled_light.js ────────────────────────────
// Scheduled Light module renderer — uses shared W.* widgets.
// ───────────────────────────────────────────────────────────────────────

(function(){
  'use strict';

  var MODULE_ID = 'scheduled_light';

  function esc(s){ return String(s==null?'':s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;'); }

  function renderScheduledLight(inst){
    ...
    return Promise.all([statusData, liveData]).then(function(results){
      ...
      var h = '';
      h += W.cardHeader('Scheduled Light', state.status || 'Idle',
  ...
  window.renderScheduledLightModule = renderScheduledLight;
