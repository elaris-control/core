// ── public/page/modules/motion_light.js ───────────────────────────────
// Motion Light module renderer — uses shared W.* widgets.
// ───────────────────────────────────────────────────────────────────────

(function(){
  'use strict';

  var MODULE_ID = 'motion_light';

  function esc(s){ return String(s==null?'':s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;'); }

  function renderMotionLight(inst){
    ...
    return Promise.all([statusData, liveData]).then(function(results){
      ...
      var h = '';
      h += W.cardHeader('Motion Light', state.status || 'Idle',
  ...
  window.renderMotionLightModule = renderMotionLight;
