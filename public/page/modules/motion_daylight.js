// ── public/page/modules/motion_daylight.js ────────────────────────────
// Motion Daylight module renderer — uses shared W.* widgets.
// ───────────────────────────────────────────────────────────────────────

(function(){
  'use strict';

  var MODULE_ID = 'motion_daylight';

  function esc(s){ return String(s==null?'':s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;'); }

  function renderMotionDaylight(inst){
    ...
    return Promise.all([statusData, liveData]).then(function(results){
      ...
      var h = '';
      h += W.cardHeader('Motion Daylight', state.status || 'Idle',
  ...
  window.renderMotionDaylightModule = renderMotionDaylight;
