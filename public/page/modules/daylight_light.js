// ── public/page/modules/daylight_light.js ─────────────────────────────
// Daylight Light module renderer — uses shared W.* widgets.
// ───────────────────────────────────────────────────────────────────────

(function(){
  'use strict';

  var MODULE_ID = 'daylight_light';

  function esc(s){ return String(s==null?'':s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;'); }

  function renderDaylightLight(inst){
    ...
    return Promise.all([statusData, liveData]).then(function(results){
      ...
      var h = '';
      h += W.cardHeader('Daylight Light', state.status || 'Idle',
  ...
  window.renderDaylightLightModule = renderDaylightLight;
