// ── public/page/modules/thermostat.js ─────────────────────────────────
// Unified renderer for ALL thermostat modules.
// Layout EXACTLY matches lighting widget.
// ───────────────────────────────────────────────────────────────────────
...
(function(){
  ...
  async function renderThermostat(inst){
    ...
    var meta   = thermoMeta(inst.module_id);
    ...
    var mid = inst.module_id, iid = inst.id;
    ...
    h += '<div>';
    h += '<div style="font-size:11px;color:var(--muted2);font-weight:800;letter-spacing:.04em;text-transform:uppercase">'+esc(meta.label)+'</div>';
    ...
    h += '<button onclick="thermoControl(\''+mid+'\','+iid+',{manual:'+(manualActive?false:true)+'})" style="padding:9px 14px;border-radius:10px;border:1px solid '+(manualActive?'rgba(245,158,11,.5)':'var(--line2)')+';background:'+(manualActive?'rgba(245,158,11,.15)':'rgba(255,255,255,.05)')+';color:'+(manualActive?'#f59e0b':'var(--text)')+';font-size:12px;font-weight:800;cursor:pointer">'+(manualActive?'Turn OFF':'Turn ON')+'</button>';
    if(manualActive) h += '<button onclick="thermoControl(\''+mid+'\','+iid+',{clear_manual:true})" style="padding:9px 12px;border-radius:10px;border:1px solid rgba(245,158,11,.28);background:rgba(245,158,11,.08);color:#f59e0b;font-size:12px;font-weight:800;cursor:pointer">Clear Manual</button>';
    h += '</div>';
    h += '</div>';

    // ═══ 2. MODE PILLS ROW — EXACT same structure as lighting ═══
    h += '<div style="display:flex;gap:6px;flex-wrap:wrap">';

    // Mode buttons — same style as lighting modeBtn (padding:7px 10px, font-size:11px, border-radius:999px)
    h += '<button onclick="thermoControl(\''+mid+'\','+iid+',{mode:\'heating\'})" style="padding:7px 10px;border-radius:999px;border:1px solid '+(mode==='heating'?'#f5c842':'var(--line)')+';background:'+(mode==='heating'?'rgba(255,255,255,.08)':'rgba(255,255,255,.03)')+';color:'+(mode==='heating'?'#f5c842':'var(--muted2)')+';font-size:11px;font-weight:800;cursor:pointer">Heating</button>';
    h += '<button onclick="thermoControl(\''+mid+'\','+iid+',{mode:\'cooling\'})" style="padding:7px 10px;border-radius:999px;border:1px solid '+(mode==='cooling'?'#1d8cff':'var(--line)')+';background:'+(mode==='cooling'?'rgba(255,255,255,.08)':'rgba(255,255,255,.03)')+';color:'+(mode==='cooling'?'#1d8cff':'var(--muted2)')+';font-size:11px;font-weight:800;cursor:pointer">Cooling</button>';
    h += '<button onclick="thermoControl(\''+mid+'\','+iid+',{mode:\'off\'})" style="padding:7px 10px;border-radius:999px;border:1px solid '+(mode==='off'?'#f59e0b':'var(--line)')+';background:'+(mode==='off'?'rgba(255,255,255,.08)':'rgba(255,255,255,.03)')+';color:'+(mode==='off'?'#f59e0b':'var(--muted2)')+';font-size:11px;font-weight:800;cursor:pointer">Off</button>';

    // Output status pill — same style as lighting (padding:4px 10px, font-size:11px, font-weight:700)
    ...
    if(meta.hasSetpoint){
      ...
      h += '<button onclick="thermoControl(\''+mid+'\','+iid+',{setpoint:'+(Math.round((setpoint-0.5)*10)/10)+'})" style="width:32px;height:32px;border-radius:50%;border:1px solid var(--line);background:rgba(255,255,255,.05);cursor:pointer;font-size:18px">-</button>';
      h += '<span style="font-size:22px;font-weight:900;min-width:56px;text-align:center;color:#f5c842">'+setpoint.toFixed(1)+'°</span>';
      h += '<button onclick="thermoControl(\''+mid+'\','+iid+',{setpoint:'+(Math.round((setpoint+0.5)*10)/10)+'})" style="width:32px;height:32px;border-radius:50%;border:1px solid var(--line);background:rgba(255,255,255,.05);cursor:pointer;font-size:18px">+</button>';
      h += '</div></div>';
    }

    // ═══ 4. MINI STAT GRID — same 3-column as lighting ═══
    h += '<div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:6px">';

    if(inst.module_id === 'call_thermostat'){
      var callRaw2 = vals.zone_1_call;
      var callDem = callRaw2==='ON'||callRaw2==='1'||callRaw2==='true';
      h += miniStat('Mode', mode.charAt(0).toUpperCase()+mode.slice(1), false);
      h += miniStat('Call', callDem?'Active':'Idle', callDem);
      h += miniStat('Output', outputOn?'On':'Off', outputOn);
    } else if(meta.isZoned){
    ...
    if(meta.isZoned){
      ...
      for(var n = 1; n <= 6; n++){
        ...
        if(zSource==='temp' && zTemp!=null){
          ...
          zoneH += '<button onclick="var p={};p[\'zone_'+n+'_setpoint\']='+sM+';thermoControl(\''+mid+'\','+iid+',p)" style="padding:2px 7px;border-radius:8px;border:1px solid var(--line2);background:rgba(255,255,255,.05);color:var(--text);font-size:11px;font-weight:800;cursor:pointer">−</button>';
          zoneH += '<span style="font-size:14px;font-weight:900;color:#f5c842;min-width:34px;text-align:center">'+effSp.toFixed(1)+'°</span>';
          zoneH += '<button onclick="var p={};p[\'zone_'+n+'_setpoint\']='+sP+';thermoControl(\''+mid+'\','+iid+',p)" style="padding:2px 7px;border-radius:8px;border:1px solid var(--line2);background:rgba(255,255,255,.05);color:var(--text);font-size:11px;font-weight:800;cursor:pointer">+</button>';
          zoneH += '</div></div>';
        } else if(zSource==='call'){
          var sM2 = Math.round((effSp-0.5)*10)/10, sP2 = Math.round((effSp+0.5)*10)/10;
          zoneH += '<div style="text-align:right"><div style="font-size:10px;color:var(--muted2);text-transform:uppercase">Setpoint'+(zSp==null?' <span style="font-size:9px;opacity:.6">(global)</span>':'')+'</div>';
          zoneH += '<div style="display:flex;align-items:center;gap:4px;justify-content:flex-end;margin-top:2px">';
          zoneH += '<button onclick="var p={};p[\'zone_'+n+'_setpoint\']='+sM2+';thermoControl(\''+mid+'\','+iid+',p)" style="padding:2px 7px;border-radius:8px;border:1px solid var(--line2);background:rgba(255,255,255,.05);color:var(--text);font-size:11px;font-weight:800;cursor:pointer">−</button>';
          zoneH += '<span style="font-size:14px;font-weight:900;color:#f5c842;min-width:34px;text-align:center">'+effSp.toFixed(1)+'°</span>';
          zoneH += '<button onclick="var p={};p[\'zone_'+n+'_setpoint\']='+sP2+';thermoControl(\''+mid+'\','+iid+',p)" style="padding:2px 7px;border-radius:8px;border:1px solid var(--line2);background:rgba(255,255,255,.05);color:var(--text);font-size:11px;font-weight:800;cursor:pointer">+</button>';
          zoneH += '</div></div>';
        } else {
          var sM3 = Math.round((effSp-0.5)*10)/10, sP3 = Math.round((effSp+0.5)*10)/10;
          zoneH += '<div style="text-align:right"><div style="font-size:10px;color:var(--muted2);text-transform:uppercase">Setpoint'+(zSp==null?' <span style="font-size:9px;opacity:.6">(global)</span>':'')+'</div>';
          zoneH += '<div style="display:flex;align-items:center;gap:4px;justify-content:flex-end;margin-top:2px">';
          zoneH += '<button onclick="var p={};p[\'zone_'+n+'_setpoint\']='+sM3+';thermoControl(\''+mid+'\','+iid+',p)" style="padding:2px 7px;border-radius:8px;border:1px solid var(--line2);background:rgba(255,255,255,.05);color:var(--text);font-size:11px;font-weight:800;cursor:pointer">−</button>';
          zoneH += '<span style="font-size:14px;font-weight:900;color:#f5c842;min-width:34px;text-align:center">'+effSp.toFixed(1)+'°</span>';
          zoneH += '<button onclick="var p={};p[\'zone_'+n+'_setpoint\']='+sP3+';thermoControl(\''+mid+'\','+iid+',p)" style="padding:2px 7px;border-radius:8px;border:1px solid var(--line2);background:rgba(255,255,255,.05);color:var(--text);font-size:11px;font-weight:800;cursor:pointer">+</button>';
          zoneH += '</div></div>';
        }
        zoneH += '</div>';
  ...
  window.renderThermostatModule = renderThermostat;
  window.isThermostatModule = function(moduleId){ return THERMO_FAMILY.has(String(moduleId)); };
